...
public class EncryptUtil {
    ...
    public static String encHashSHA256(String str) throws Exception {
        ...
        String plantText = addMessage + str; // 암호화 시킬 값에 보안강화를 위해 임의 값(addMessage)을 추가함 !중요

        try {

            // 자바는 기본적으로 표준 암호화 알고리즘을 java.security 패키지를 통해 제공함
            // 여러 해시 알고리즘 중 가장 많이 사용되는 SHA-256를 지원하고 있음
            MessageDigest sh = MessageDigest.getInstance("SHA-256"); // sha 알고리즘 호출

            sh.update(plantText.getBytes()); // string 을 byte형태로 바꿈
     ...
     */
    //CBC : 16단위로 암호화 시킴
    //      사용자의 입력값이 많아지면 속도가 느려지게 되는데 이를 보완
    //      블럭 단위로 데이터를 처리하기 때문에 처리속도가 빠름
